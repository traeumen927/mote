...
protocol GoogleSignInPresenterStoreType: AnyObject {
    var presentingViewController: UIViewController? { get set }
}

final class GoogleSignInPresenterStore: GoogleSignInPresenterStoreType {
...
final class GoogleAuthService: GoogleAuthServicing {
    ...
    private weak var presenterStore: GoogleSignInPresenterStoreType?
    
    init(presenterStore: GoogleSignInPresenterStoreType) {
        self.presenterStore = presenterStore
    }
    
    @MainActor
    func signIn(completion: @escaping (Result<User, Error>) -> Void) {
        guard let presentingViewController = self.presenterStore?.presentingViewController else { return }
